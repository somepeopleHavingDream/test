...
//        System.out.println(person1);
    ...
    private static void test() {
        ...
        try {
            ...
            // 这个地方，jackson是真的拿不到，但是fastjson可以拿到
            String dataset = JSON.parseObject(body).getString("dataset");
//            String dataset = objectMapper.readTree(body).get("dataset").asText();
            System.out.println("dataset: " + dataset);
            String rows = JSON.parseObject(dataset).getString("rows");
//            String rows = objectMapper.readTree(dataset).get("rows").asText();
